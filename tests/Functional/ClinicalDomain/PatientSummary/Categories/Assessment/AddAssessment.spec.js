...
test.describe("LifeStyle Category", () => {
    test("Add LifeStyle", async ({ page }) => {
      ...
      for (const data of jsonData.PatientDetails) {
        ...
        await contacthistory.enterContactDate('16/05/2025');
 ...
 await page.pause()
  ...
  await page.pause()
  await assessmentExtraDetails.enterOnSetDate(jsonData.AddDiagnosis[index].diag_date_onset.toString());
  await assessmentExtraDetails.enterDiagnosedDate(jsonData.AddDiagnosis[index].diag_date_diagnosed.toString());
  await assessmentExtraDetails.enterDiagnosis1stSeenDate(jsonData.AddDiagnosis[index].diag_date_firstseen)
  await assessmentExtraDetails.selectSeverity(jsonData.AddDiagnosis[index].diag_severity)
  await assessmentExtraDetails.selectStatus(jsonData.AddDiagnosis[index].diag_diagnosis_status)
  await assessmentExtraDetails.selectActivity(jsonData.AddDiagnosis[index].diag_activity)
  await assessmentExtraDetails.selectCountryOfDiagnosis("India")
  await assessmentExtraDetails.searchAndSelectUnderlayingCause("Asthma")
  await page.waitForTimeout(1000)
  await assessmentExtraDetails.searchAndSelectComplicationsAndDiagnosis("Dengue haemorrhagic fever")
  await assessmentExtraDetails.searchAndSelectExternalCause("Abnormal findings on diagnostic imaging of other body structures")
  await assessmentExtraDetails.enterClinicalItemNotes("Added Diagnosis Notes From Playwright");
  ...
  await page.waitForTimeout(2000);
  await assessment.ClickOnbtnKFT()
  await page.waitForTimeout(2000);
  await assessmentExtraDetails.clickOnSaveExtraDetails();
  ...
  await page.waitForTimeout(2000);
  await assessment.ClickOnbtcRecomToRest()
  await page.waitForTimeout(2000);
...
if (match) {
  ...
  // Edit presenting lifestyle
      await page.waitForTimeout(1000);
      await lifestyle.clickOnItemEdit('Current Smoker');
      await page.waitForTimeout(2000);
