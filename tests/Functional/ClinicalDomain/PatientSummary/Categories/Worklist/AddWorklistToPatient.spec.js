...
test.describe("Worklist Category", () => {
    test("Add Worklist", async ({ page }) => { 
  ...
  await page.pause()
        ...
        await patientsummary.selectWorklistlink();
        await page.waitForTimeout(1000);
        await patientsummary.setWorklist();
        await page.waitForTimeout(1500);
        await patientsummary.setWorklistPriority();
       await page.pause()
        await patientsummary.addworklistStartDate(jsonData.AddWorklist[index].start_date);
        await patientsummary.addworklistEndDate(jsonData.AddWorklist[index].end_date);
        await page.pause()
        await patientsummary.expandWorklist();
        await page.waitForTimeout(1000);
        await page.pause()
        await patientsummary.addworTasDueTime(jsonData.AddWorklist[index].due_time);
        await patientsummary.addWorklistNotes(jsonData.AddWorklist[index].notes);
...
if (match) {
  ...
  console.log(
    "\n Patient Clinical Records Comparision: Parameters from both JSON files do not match!\n"
      ...
      //await page.locator("xpath=//button[@data-testid='logout']").click()
